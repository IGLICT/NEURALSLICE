...
class Dataset(data.Dataset):
    ...
    def __init__(self, split_type, opt, encoder_type='points', is_small=True, img_num=0):
        '''
        ...
        with open('dataset/mysplit.json', 'r') as outfile:  
        ...
        # # for cat in mycats:
        # models = list(split[split_type][mycats])
        # for model in models:
        #     self.models.append(model)       # All models used for training
        #     self.model2cat[model] = mycats
    ...
    def __getitem__(self,index):
        ...
        if self.encoder_type == 'image':
            ...
            mask = np.random.randint(0,X.shape[0],2500)  # Randomly sample 1024 points for training of PointsSVR network
            ...
            try:
                # X = np.load('./data/ShapeNet/'+'{}/{}/pointcloud.npz'.format(self.model2cat[modelfile],modelfile), allow_pickle=True)
                # X = X['points']
                X = np.load(self.dataset_dir+'{}/{}/points.npy'.format(self.model2cat[modelfile],modelfile), allow_pickle=True)   # Read point cloud 
                # N = np.load(self.dataset_dir+'{}/{}/est_normals.npy'.format(self.model2cat[modelfile], modelfile), allow_pickle=True)
            except:
                # Incase fail to load the file, simply return the first file of dataset to prevent crashing.
                ...
                N = np.load(self.dataset_dir+'{}/{}/normals.npy'.format(self.model2cat[modelfile], modelfile), allow_pickle=True)
                
            X /= np.max(np.linalg.norm(X, axis=1))   # Point cloud to lie withing a unit sphere
            mask = np.random.randint(0,X.shape[0],2500)  # Randomly sample 2562 points for training NMF AE
            L_mask = np.random.randint(0,X.shape[0],25600)
            X_m = torch.from_numpy(X[mask,...]).float() 
            X = torch.from_numpy(X[L_mask,...]).float()

            # N = torch.from_numpy(N[mask,...]).float()
            
            return X_m, X, self.model2cat[modelfile], modelfile 
